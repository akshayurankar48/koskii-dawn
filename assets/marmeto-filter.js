...
marmeto.Collection = (function() {
  ...
  Collection.prototype.initCustomFilters = function() {
    ...
    return {
      ...
      material: function(ortags, product) {
        var or = false;

        ortags.filter(function(tag) {
          if(product.metafields.material && _this.helpers().handleize(tag) == _this.helpers().handleize(product.metafields.material)) or = true;
        })
        return or;
      }
    }
    ...
    return {
      renderProduct: function (index, product) {
        ...
        if(product.featured_image) {
          ...
          image_space = 100 / product.media[0].aspect_ratio;
        }
        ...
        if (product.compare_at_price != null) {   
          productHtml += '<div class="mmc-product-badge">SALE</div>';
        }
        ...
        if(product.compare_at_price > product.price) {
          ...
          productHtml += '<span class="mmc-price-discount"> ( '+Math.trunc( discount )+'% off )</span>';
        } else {
        ...
        productHtml += '</div>';
        productHtml += '</div>';
        productHtml += '<a href="#" class="mmc-card-buy_button">ADD TO CART</a>'; 
        productHtml += '</div>';
